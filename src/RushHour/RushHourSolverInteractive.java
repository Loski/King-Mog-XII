...
public class RushHourSolverInteractive extends JFrame{
	...
	public void afficherResultat(Object[] result)
	{
		...
		objWrapper.add(new JLabel("R�sultat pour le fichier "+this.fileLoaded),BorderLayout.NORTH);
	...
	public JPanel panelObj(Object[] result)
	{
		...
		else
		{
			...
			nbCase = new JLabel("Avec "+result[0]+" cases d�plac�es");
		...
		if(result.length==2)
		{
			...
			pan.add(new JLabel("Nombre de configurations r�alisables : "+(this.g.getConfigurations().size()-1)));
		}
		
		pan.add(new JLabel("Temps d'�x�cution de l'ALGO : SOON"));
	...
	public JMenuBar createMenu()
	{
		  ...
		  RHMGuro.addActionListener(new ActionListener() { 
			  public void actionPerformed(ActionEvent e) { 
				  ...
				  {
					  JOptionPane.showMessageDialog(new JFrame(), "Gurobi n'est pas install�", "ERREUR",
						        JOptionPane.ERROR_MESSAGE);  
				  }
				  } 
				} );
